...
enum SBP2TaskManagementFunction: UInt64, CaseIterable, Identifiable {
    ...
    var displayName: String {
        switch self {
        case .abortTaskSet: return "Abort Task Set"
        case .logicalUnitReset: return "Logical Unit Reset"
        case .targetReset: return "Target Reset"
        }
    }
}
...
extension ASFWDriverConnector {
    ...
    @discardableResult
    func submitSBP2TaskManagement(handle: UInt64, function: SBP2TaskManagementFunction) -> Bool {
        guard isConnected else {
            log("submitSBP2TaskManagement: Not connected", level: .warning)
            return false
        }

        var inputs: [UInt64] = [handle, function.rawValue]

        let kr = inputs.withUnsafeMutableBufferPointer { buffer -> kern_return_t in
            IOConnectCallScalarMethod(
                connection,
                Method.submitSBP2TaskManagement.rawValue,
                buffer.baseAddress,
                UInt32(buffer.count),
                nil,
                nil)
        ...
        guard kr == KERN_SUCCESS else {
            let errorMsg = "submitSBP2TaskManagement failed: \(interpretIOReturn(kr))"
            log(errorMsg, level: .error)
            lastError = errorMsg
            return false
        }

        log(String(format: "SBP2 task management submitted (handle=0x%llX, function=0x%02llX)",
                   handle, function.rawValue),
            level: .success)
        return true
    }
